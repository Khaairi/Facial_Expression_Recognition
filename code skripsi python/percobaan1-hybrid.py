...
def create_dataloaders(train_dataset, val_dataset, test_dataset, sampler):
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, 
                             generator=torch.Generator().manual_seed(SEED), sampler=sampler, num_workers=4)
...
def train_model(model, train_loader, val_loader):
    ...
    for epoch in range(EPOCHS):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            model_path = os.path.join(SAVE_PATH, "percobaan1-hybrid_best.pt")
            torch.save({
...
def main():
    ...
    checkpoint = torch.load("../Hasil Eksperimen/percobaan1-hybrid_best.pt")
